...
\begin{solution}
  ...
  \begin{enumerate}
    ...
    \item
      ...
      is positive. By the quadratic formula, the roots of the denomiator are
      \begin{equation*}
    ...
    \item
      ...
      $(P_r \ast f)(t)$ and $(Q_r \ast f)(t)$ are conjugate harmonic. $f$ gives the boundary values for $u$ in the sense that the values for $u$ on
      the unit circle are given by $f$ and its conjugate function.
  \end{enumerate}
...
\begin{proof}
  ...
  First, assume $f$ is a trigonometric polynomial. Then we can write $f(t) = \sum_{l \in \Z} c_l e^{2 \pi i l t}$ and $\hat{f}(0) = c_0$. Define $g_n(t) = g(nt)$. $g \in
  L^{\infty} \subset L^2$, so we can write the Fourier expansion of $g$
